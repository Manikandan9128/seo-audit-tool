"""Runtime-editable settings (Gemini + Groq + Claude API keys) backed by
the app_settings table, so the user can change them from the UI without
editing .env or restarting the server. Any one key alone is enough — see
app.integrations.text_ai_client for the fallback logic that picks
...
from app.integrations.text_ai_client import GROQ_API_URL, GROQ_MODEL, OPENROUTER_API_URL, OPENROUTER_MODEL, _reserve_gemini_slot
...
def test_sheets_connection(db: Session) -> dict:
    """Tests the connected Sheets OAuth account by creating + deleting a
    throwaway spreadsheet — confirms Sheets API, Drive API, and the
    connection are all actually working together."""
    try:
